...
arma::mat sigma2(c, mcmc_samples); sigma2.fill(0.00);
...
arma::vec shape_update = n*(d-1)/2.00 +
                         a_sigma2;
...
if(alpha_init.isNotNull()){
  alpha = Rcpp::as<arma::mat>(alpha_init);
  }
...
arma::colvec sigma2_col = sigma2.col(0);  
arma::vec diag_vals;                     
for(int j = 0; j < c; ++j){
  
   arma::vec rep_block(n(j), arma::fill::value(1.00/sigma2_col(j)));
...
for(int j = 1; j < mcmc_samples; ++j){
   ...
   for(int k = 0; k < (d-1); ++k){
      ...
      //alpha
      alpha.col(k) = alpha_update(X_star,
                                  sum_n,
                                  kappa.col(k),
                                  omega.col(k),
                                  theta.slice(j).col(k),
   ...
   arma::colvec sigma2_col = sigma2.col(j);  
   arma::vec diag_vals;                     
   for(int k = 0; k < c; ++k){
